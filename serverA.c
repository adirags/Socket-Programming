...
int main(void) {
	...
	getpeername(sockID, (struct sockaddr*)&addr, &len2);
	struct sockaddr_in *s3 = (struct sockaddr_in *)&addr;
	TCPport = ntohs(s3->sin_port);
	inet_ntop(AF_INET, &s3->sin_addr, ipstr, sizeof ipstr);
	...
	printf("For this connection with the client, the Server A has TCP port number = %d and IP address = %s\n",ntohs(sin.sin_port),inet_ntop(P->ai_family, get_internet_address((struct sockaddr *)P->ai_addr), s, sizeof s));
	...
	if((rv = getaddrinfo(NULL, "21141", &hints, &serverInfo)) != 0) {//if getaddrinfo didn't succeed
		fprintf(stderr,"getaddrinfo : %s\n", gai_strerror(rv));
		return 1;
		}
	...
	if (getsockname(sockID, (struct sockaddr *)&sin3, &len3) == -1)
		perror("getsockname");
	printf("For this connection with the client, the Server A has the UDP port number = %d, and IP address = %s\n",ntohs(sin3.sin_port),s);
	close(sockID);
	return 0;
	}
