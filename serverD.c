...
******************/
int main(void) {
	printf("Server D is up and running\n\n");
	pid_t pid;
	int i, j, k, l, yes = 1;
	...
	if((rv = getaddrinfo(NULL, "24141", &hints, &serverInfo)) != 0) {//if getaddrinfo didn't succeed
		fprintf(stderr,"getaddrinfo : %s\n", gai_strerror(rv));
		return 1;
		}
	...
	if (getsockname(sockID, (struct sockaddr *)&sin3, &len3) == -1)
		perror("getsockname");
	printf("For this connection with the client, the Server D has the UDP port number = %d, and IP address = %s\n",ntohs(sin3.sin_port),s);
